...
import random
import string
print("Welcome to world of encryption from the house of Ronak Jitendrbhai Patel  \n")
print("Please select from below options: \n")
print("1) Type 1 to encrypt a new file:")
print("2) Type 2 for decrypting a file :")
options = int(input())
...
def Encrypt(filename):
    ...
    key_file.close()
    print("\n Key file has been genereated DO NOT LOSE IT KEEP IT SAFE file name is 'my_key.key'.")
    print("DO Keep it safe to access the file.\n")

    with open(Folder_Location + 'my_key.key', 'r') as my_key:
        key_data = my_key.read()

    print("Encryption process is  going on\nPlease wait.")
...
def Decrypt():
    filename_with_location = input(" Please enter filename with double quotations and with full path or location location.\n")
